...
std::string KeccakF::getDescription() const
{
    std::stringstream a;
    if ((nrRounds == nominalNrRounds) && (startRoundIndex == 0))
        a << "Keccak-f[" << std::dec << width;
    else if ((startRoundIndex + nrRounds) == nominalNrRounds)
        a << "Keccak-p[" << std::dec << width << ", " << nrRounds;
    else {
        a << "Keccak-f[" << std::dec << width;
        a << ", " << std::dec << nrRounds << " rounds " << startRoundIndex << "-" << (startRoundIndex+nrRounds-1);
    }
    a << "]";
    return a.str();
}

std::string KeccakF::getName() const
{
    std::stringstream a;
    a << "KeccakF-" << std::dec << width << "-" << nrRounds;
    if (startRoundIndex != 0)
        a << "-" << startRoundIndex;
    return a.str();
}

std::string KeccakF::buildFileName(const std::string& prefix, const std::string& suffix) const
{
...
std::string KeccakF::buildBitName(const std::string& prefixSymbol, unsigned int laneSize, unsigned int z)
{
    std::stringstream s;
    s << prefixSymbol;
    if (laneSize > 1) {
...
std::string KeccakF::bitName(const std::string& prefix, unsigned int x, unsigned int y, unsigned int z) const
{
    return buildBitName(laneName(prefix, x, y), laneSize, z);
}

std::string KeccakF::laneName(const std::string& prefix, unsigned int x, unsigned int y)
{
    return prefix + "bgkms"[y] + "aeiou"[x];
}

std::string KeccakF::sheetName(const std::string& prefix, unsigned int x)
{
...
std::string KeccakP::getName() const
{
    std::stringstream a;
    a << "KeccakP-" << std::dec << width << "-" << nrRounds;
    return a.str();
}
...
std::string KeccakPStar::getName() const
{
    std::stringstream a;
    a << "KeccakPStar-" << std::dec << width << "-" << nrRounds;
    return a.str();
}
