...
const std::vector<UINT8>& MessageBlock::get() const
{
    return block;
}
...
const std::vector<UINT8>& MessageQueue::firstBlock() const
{
...
std::ostream& operator<<(std::ostream& a, const PaddingRule& pad)
{
...
std::string SimplePadding::getDescription() const
{
    return "pad10*";
}
...
std::string MultiRatePadding::getDescription() const
{
    return "pad10*1";
}
...
std::string OldDiversifiedKeccakPadding::getDescription() const
{
    std::stringstream str;
    str << "oldKeccakPadding[d=" << std::dec << (int)diversifier << "]";
    return str.str();
}
