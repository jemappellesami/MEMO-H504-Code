...
public:
    ...
    /** Method that returns a reference to the block.
      ...
      * last few bits in its least significant bits.
      */
    const std::vector<UINT8>& get() const;
};
...
public:
    ...
    /** Method that returns a reference to the first block of the sequence.
      ...
      * last few bits in its least significant bits.
      */
    const std::vector<UINT8>& firstBlock() const;
...
public:
    ...
    /**
      ...
      */
    friend std::ostream& operator<<(std::ostream& a, const PaddingRule& pad);
protected:
...
template<class InputIterator>
std::vector<UINT8> getKeyPack(InputIterator keyStart, unsigned int keyLengthInBits, unsigned int packLengthInBits)
{
