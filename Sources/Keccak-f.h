...
public:
    ...
    /**
      ...
      * a suffix to getName().
      */
    std::string buildFileName(const std::string& prefix, const std::string& suffix) const;
    /**
    ...
    /**
      ...
      *                 actual evaluation.
      */
    template<class Lane> void forward(std::vector<Lane>& state) const;
    /**
    ...
    /**
      ...
      */
    static std::string buildBitName(const std::string& prefixSymbol, unsigned int laneSize, unsigned int z);
    /**
      ...
      * @param z        The z coordinate.
      */
    std::string bitName(const std::string& prefix, unsigned int x, unsigned int y, unsigned int z) const;
    /**
      ...
      * @param y        The y coordinate.
      */
    static std::string laneName(const std::string& prefix, unsigned int x, unsigned int y);
    /**
      ...
      * @param x        The x coordinate.
      */
    static std::string sheetName(const std::string& prefix, unsigned int x);
private:
    /**
...
template<class Lane>
void KeccakF::forward(std::vector<Lane>& state) const
{
...
template<class Lane>
void KeccakF::inverse(std::vector<Lane>& state) const
{
...
template<class Lane>
void KeccakF::round(std::vector<Lane>& state, int roundIndex) const
{
    theta(state);
    rho(state);
    pi(state);
...
template<class Lane>
void KeccakF::inverseRound(std::vector<Lane>& state, int roundIndex) const
{
    iota(state, roundIndex);
    inverseChi(state);
    inversePi(state);
...
template<class Lane>
void KeccakF::chi(std::vector<Lane>& A) const
{
    std::vector<Lane> C(5);
...
template<class Lane>
void KeccakF::inverseChi(std::vector<Lane>& A) const
{
    for(unsigned int y=0; y<5; y++) {
...
template<class Lane>
void KeccakF::theta(std::vector<Lane>& A) const
{
    std::vector<Lane> C(5);
    for(unsigned int x=0; x<5; x++) {
        C[x] = A[index(x,0)];
...
template<class Lane>
void KeccakF::inverseTheta(std::vector<Lane>& A) const
{
    std::vector<Lane> C(5);
    for(unsigned int x=0; x<5; x++) {
        C[x] = A[index(x,0)];
...
template<class Lane>
void KeccakF::pi(std::vector<Lane>& A) const
{
    std::vector<Lane> a(A);
    for(unsigned int x=0; x<5; x++)
    for(unsigned int y=0; y<5; y++) {
...
template<class Lane>
void KeccakF::inversePi(std::vector<Lane>& A) const
{
    std::vector<Lane> a(A);
    for(unsigned int X=0; X<5; X++)
    for(unsigned int Y=0; Y<5; Y++) {
...
template<class Lane>
void KeccakF::rho(std::vector<Lane>& A) const
{
    for(unsigned int x=0; x<5; x++)
    for(unsigned int y=0; y<5; y++)
...
template<class Lane>
void KeccakF::inverseRho(std::vector<Lane>& A) const
{
    for(unsigned int x=0; x<5; x++)
    for(unsigned int y=0; y<5; y++)
        ROL(A[index(x,y)], -rhoOffsets[index(x,y)]);
...
template<class Lane>
void KeccakF::iota(std::vector<Lane>& A, int roundIndex) const
{
